...
A：
  - 一层数据格式的 转化器/适配器，把传入的数据参数经过封装后，返回一个ReactElement对象
  - 通过React.createElement这一层中转函数，可以在创建ReactElement对象时，不用一次传入很多函数参数

具体的细节内容，大致有以下几步
  - 从config对象中 依次读取 key/ref/self/source
  - 筛选config中的某些属性，作为props内的属性
  - 根据 children长度，构造出 children Array/对象
  - 根据 是否有defaultProps，赋值 props内的属性值
  - 返回一个 ReactElement(type, key, ref, self, source, ReactCurrentOwner.current, props)
...
export function createElement(type, config, children) {
  ...
  //S5 最后返回一个调用ReactElement执行方法，并传入刚才处理过的参数
  return ReactElement(
    type,
    key,
    ref,
